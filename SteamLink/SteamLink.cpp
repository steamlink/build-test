...
void print_packet(uint8_t* packet, uint8_t packet_length) {
	...
	if (is_data) {
		...
		uint8_t qos = ((data_header*) packet)->qos;
		...
		uint8_t qos = ((control_header*) packet)->qos;
...
void print_op_code(uint8_t op) {
	...
	if (op == SL_OP_SR) Serial.print("SR");	
	...
	if (op == SL_OP_AK) Serial.print("AK");
	if (op == SL_OP_NK) Serial.print("NK");
